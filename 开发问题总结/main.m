...
*********************************************开发问题目录*************************************************
 ...
 49 > 用AFN进行接口请求的时候, 一直报错请求超时, -1001和 UserInfo={_kCFStreamErrorCodeKey=-2102等
...
// 41 > @class的用法详解
        /*
            @class引入头文件就是为了告诉编译器有这个类名, 但是这个类所有的方法属性都不能调用, 一般用在.h中声明这个类
            1 > 比如A类的.h文件中需要声明一个B类的, 也就是在A类的.h中写上 @property (nonatomic, strong)B *b;
                如果在A类的.m文件中需要对这个B类进行 alloc init的话, 就需要在A的.m头文件用import引入B类, 这么做看起来
                感觉有点麻烦, 因为A的.h中已经class了B, 还在A的.m中继续import, 但是这么做的好处是不把B类里面的内容暴露出来
         
            2 > 如果两个类相互用import引用的话, 在.m中相互引用没事, 但是在.h中相互引用的话就会报错, 这个时候把其中的一个
                改成@class引入就可以了
         
         总结 : @class一般就是用在.h文件中, 并且只是告诉编译器有这个类名, 这样引入的类的方法属性等的都不能使用, 比如@class引
...
// 49 > 用AFN进行接口请求的时候, 一直报错请求超时, -1001和 UserInfo={_kCFStreamErrorCodeKey=-2102等
        /*
            确认添加ATS后, 然后看运行的设备(真机或者模拟器)是否和后台电脑接入的是同一无线网下
